...
public class LoginPage extends VerticalLayout {
    ...
    public int postLogin(User UserNuevo){
        try {
            ...
            request = HttpRequest.newBuilder()
                    .uri(URI.create("http://api:8887/users"))
    ...
    public int postSignup(User UserNuevo){
        try {
            ...
            String json = objectMapper.writeValueAsString(UserNuevo);
            request = HttpRequest.newBuilder()
                    .uri(URI.create("http://api:8887/usersp"))
                    .header("Content-Type", "application/json")
                    .method("POST", HttpRequest.BodyPublishers.ofString(json))
                    .build();
            response = client.send(request, HttpResponse.BodyHandlers.ofString());
    ...
    public LoginPage() {
        ...
        loginButton.addClickListener(event -> {
            ...
            User nu = new User(username.getValue(), password.getValue());
            int us = postLogin(nu);
            if (us == 200) {
            ...
            Button register = new Button("Registrar",
                    event2 -> {
                        ...
                        if (Objects.equals(password3.getValue(), password2.getValue())) {
                            User sign = new User(username2.getValue(), password2.getValue());
